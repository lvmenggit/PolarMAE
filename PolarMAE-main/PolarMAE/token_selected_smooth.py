...
class TokenSelect_smooth(nn.Module):
    ...
    def get_score(self, a: torch.Tensor, b: torch.Tensor):
        if self.distance == "cosine":
            ...
            dist = torch.sum(torch.abs(a.unsqueeze(2) - b.unsqueeze(1)), dim=-1)
    ...
    def token_initialization(self, token: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            select_index = (torch.tensor(select_index).unsqueeze(0).unsqueeze(-1).to(device=token.device)).expand(
                token.shape[0], x, token.shape[2],
            )
            unselect_index = (torch.tensor(unselect_index).unsqueeze(0).unsqueeze(-1).to(device=token.device)).expand(
                token.shape[0], token.shape[1] - x, token.shape[2],
            )

        select_token = token.gather(dim=1, index=select_index)
        unselect_token = token.gather(dim=1, index=unselect_index)

        assert select_token.shape[1] + unselect_token.shape[1] == (self.token_num - 1), "Wrong shape!"
        assert select_index.shape[1] + unselect_index.shape[1] == (self.token_num - 1), "Wrong shape!"
    ...
    def token_expansion(
        ...
        """
        Fixes for ROI-only selection:
        - token_num uses current subset length (select+unselect), not full L
        - clamp expansion numbers to remaining unselect size
        - fix batch bug in stage==2/4 (no edge_idx[1] hacks)
        - stable normalization with eps
        """
        # IMPORTANT: ROI-subset token count
        self.token_num = int(select_token.shape[1] + unselect_token.shape[1])

        for stage in range(1, 2):  # keep your original behavior
            target_keep = int(self.token_num * self.keep_rate[stage - 1])
            init_keep = int(self.token_num * self.initialization_keep_rate)
            expansion_token_num = max(0, target_keep - init_keep)
            expansion_token_num = min(expansion_token_num, unselect_token.shape[1])

            for k in range(1, self.expansion_multiple_stage + 1):
                if unselect_token.shape[1] <= 0 or expansion_token_num <= 0:
                    continue
                ...
                if k == self.expansion_multiple_stage:
                    ...
                    multiple_expansion_token_num = expansion_token_num // self.expansion_multiple_stage

                multiple_expansion_token_num = min(multiple_expansion_token_num, unselect_token.shape[1])
                if multiple_expansion_token_num <= 0:
                    continue

                with torch.no_grad():
                    select_token_norm = F.normalize(select_token, dim=-1, eps=1e-6)
                    unselect_token_norm = F.normalize(unselect_token, dim=-1, eps=1e-6)
                    scores = self.get_score(unselect_token_norm, select_token_norm)  # [B, U, S]
                    node_max, _ = scores.max(dim=-1)                                  # [B, U]
                    edge_idx = node_max.argsort(dim=-1, descending=False)             # [B, U]
                    ...
                    if self.expansion_stage == 1:
                        ...
                        near_node_num = max(0, min(near_node_num, multiple_expansion_token_num))
                        far_node_num = multiple_expansion_token_num - near_node_num

                        near_node_indices = edge_idx[..., -near_node_num:] if near_node_num > 0 else edge_idx[:, :0]
                        far_node_indices = edge_idx[..., :far_node_num] if far_node_num > 0 else edge_idx[:, :0]
                        add_node_index = torch.cat((near_node_indices, far_node_indices), dim=-1)  # [B,K]

                        take = torch.zeros_like(edge_idx, dtype=torch.bool)
                        take.scatter_(1, add_node_index, True)
                        unadd_node_index = edge_idx[~take].view(edge_idx.shape[0], -1)
                        ...
                        far_node_num = max(0, min(far_node_num, multiple_expansion_token_num))
                        random_node_num = multiple_expansion_token_num - far_node_num

                        far_node_indices = edge_idx[..., :far_node_num]
                        rest = edge_idx[..., far_node_num:]

                        if random_node_num > 0 and rest.shape[-1] > 0:
                            total = rest.shape[-1]
                            perm = torch.randperm(total, device=edge_idx.device)
                            pick = perm[:random_node_num]
                            random_node_index = rest[..., pick]
                        else:
                            random_node_index = edge_idx[:, :0]
                        ...
                        total = edge_idx.shape[-1]
                        perm = torch.randperm(total, device=edge_idx.device)
                        add_pos = perm[:multiple_expansion_token_num]
                        unadd_pos = perm[multiple_expansion_token_num:]
                        add_node_index = edge_idx[..., add_pos]
                        unadd_node_index = edge_idx[..., unadd_pos]

                add_index = unselect_index.gather(
                    dim=1,
                    index=add_node_index.expand(unselect_token.shape[0], multiple_expansion_token_num),
                )
                ...
                new_u = max(0, unselect_token.shape[1] - multiple_expansion_token_num)
                if new_u > 0:
                    unselect_index = unselect_index.gather(
                        dim=1,
                        index=unadd_node_index.expand(unselect_token.shape[0], new_u),
                    )
                    ...
                    unselect_index = unselect_index[:, :0]
                    unselect_token = unselect_token[:, :0]
