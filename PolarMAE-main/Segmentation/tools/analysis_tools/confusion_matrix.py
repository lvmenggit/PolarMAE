...
def calculate_confusion_matrix(dataset, results):
    ...
    for idx, per_img_res in enumerate(results):
        ...
        gt_segm = dataset[idx]['data_samples'] \
            .gt_sem_seg.data.squeeze().numpy().astype(np.int64)
        res_segm = np.asarray(res_segm).astype(np.int64)
        gt_segm, res_segm = gt_segm.flatten(), res_segm.flatten()

        to_ignore = gt_segm == ignore_index
        valid_gt = (gt_segm >= 0) & (gt_segm < n)
        valid_pred = (res_segm >= 0) & (res_segm < n)
        valid_mask = (~to_ignore) & valid_gt & valid_pred

        gt_segm, res_segm = gt_segm[valid_mask], res_segm[valid_mask]
