...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            in_chans: int = 3,
            num_classes: int = 59,  # 修改为新数据集的类别数
...
class VisionTransformer_timm(VisionTransformer):
    ...
    def init_weights(self):
        ...
        if isinstance(pretrained, str):
            ...
            # Remove head weights if shape mismatch (e.g. different number of classes)
            if 'head.weight' in state_dict and state_dict['head.weight'].shape != self.head.weight.shape:
                print(f"Removing head.weight due to shape mismatch: {state_dict['head.weight'].shape} vs {self.head.weight.shape}")
